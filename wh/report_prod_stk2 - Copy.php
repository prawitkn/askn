    ...
    <div class="box box-primary">
        ...
        <div class="box-tools pull-right">
          ...
          <?php
			...
			$sql = "SELECT th.id, th.closingDate FROM stk_closing th WHERE th.statusCode='A' AND th.closingDate<='$dateFromYmd' LIMIT 1
          	";
          	$stmt = $pdo->prepare($sql);		
			$stmt->execute();
			$LPCD = $stmt->fetch()['closingDate'];
			...
			//balance
			$sql = "UPDATE tmpStock 
			SET `balanceReCheck`=`openAcc`+`openTrans`+`receive`-`sent`-`return`-`delivery`
          	";
          	$stmt = $pdo->prepare($sql);		
			$stmt->execute();
			...
			$sql = "SELECT  
			sb.`prodId`, sb.`sloc`, sb.`openAcc`, sb.`openTrans`, sb.`onway`, sb.`receive` ,sb.`sent`,sb.`return` ,sb.`delivery` ,sb.`balance` ,sb.`balanceReCheck` ,sb.`book` 	
			...
			<div class="">
				...
				?>
                  ...
                  <tr class="<?=$bgColor;?>">
					...
					<?php if($isNotEqual){ echo "/".number_format($row['balanceReCheck'],2,'.',','); } ?>
					</td>
					...
					<td style="text-align: right; color: orange;">
						<a target="_blank" href="report_prod_stk_onway.php?prodId=<?=$row['prodId'];?>&sloc=<?=$row['sloc'];?>" /><?= number_format($row['onway'],0,'.',','); ?></a>			
					</td>
					<!--<td><?= $row['catCode']; ?></td>-->
					<td style="text-align: right; font-size: small;"><?= number_format($row['openAcc']+$row['openTrans'],0,'.',','); ?></td>
