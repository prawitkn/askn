...
if(isset($_GET['sync']) AND isset($_GET['sendDate'])  ){
  ...
  switch($s_userGroupCode){ 
	...
	$row_no=1; while ($row = $stmt->fetch()) { 
		$sendNo = 'SD-'.substr(str_shuffle(MD5(microtime())), 0, 7);
		...
		$stmt2->bindParam(':sdNo', $sendNo);
		$stmt2->bindParam(':refNo', $row['sendID']);	
		$stmt2->bindParam(':sendDate', $row['issueDate']);	
		$stmt2->bindParam(':fromCode', $row['fromCode']);	
		$stmt2->bindParam(':toCode', $row['toCode']);	
		$stmt2->bindParam(':remark', $row['sendNo']);	
		$stmt2->bindParam(':s_userId', $s_userId );	
		$stmt2->execute();
		
		/*$sql = "INSERT INTO send_detail 
		(`prodItemId`, `prodId`, `prodCode`, `barcode`, `issueDate`, `machineId`, `seqNo`, `NW`, `GW`
		, `qty`, `packQty`, `grade`, `gradeDate`, `refItemId`, `itemStatus`, `remark`, `problemId`, `sdNo`)
		SELECT `prodItemId`, `prodId`, `prodCode`, `barcode`, `issueDate`, `machineId`, `seqNo`, `NW`, `GW`
		, `qty`, `packQty`, `grade`, `gradeDate`, `refItemId`, `itemStatus`, `remark`, `problemId`, :sendNo
		FROM send_production_detail 
		WHERE sendId=:sendId		
		";*/
		$sql = "INSERT INTO send_detail 
		(`prodItemId`, `sdNo`)
		SELECT `prodItemId`, :sendNo
		FROM send_production_detail 
		WHERE sendId=:sendId		
		";					
		$stmt2 = $pdo->prepare($sql);
		$stmt2->bindParam(':sendNo', $sendNo);
		$stmt2->bindParam(':sendId', $row['sendID']);	
		$stmt2->execute();
	 $row_no+=1; } //while loop
