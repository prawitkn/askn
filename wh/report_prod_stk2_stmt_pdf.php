		...
		if( isset($_GET['dateFrom']) ){
			...
			*/
			...
			$html .='<tr class="'.$bgColor.'">
				...
				//Receive Doc No
				$sql = "SELECT th.rcNo as docNo, th.receiveDate as issueDate, SUM(itm.qty) as sumQty FROM product_item itm 
	          				INNER JOIN receive_detail td ON td.prodItemId=itm.prodItemId 
	         				INNER JOIN receive th ON th.rcNo=td.rcNo AND th.statusCode='P' AND th.toCode=:toCode 
	         					AND th.receiveDate > '$lpcDate' AND th.receiveDate <= '$dateFromYmd'
	         				WHERE itm.prodCodeId=:prodCodeId 
	          				GROUP BY 1, 2 ";
					$sql.="ORDER BY 1  ";
					//$sql.="LIMIT $start, $rows ";
					$stmt3 = $pdo->prepare($sql);	
					$stmt3->bindParam(':toCode', $row['sloc']);	
					$stmt3->bindParam(':prodCodeId', $row['prodId']);	
					$stmt3->execute();
					...
					//Sent Doc No
				$sql = "SELECT th.sdNo as docNo, th.sendDate as issueDate, SUM(itm.qty) as sumQty FROM product_item itm 
	          				INNER JOIN send_detail td ON td.prodItemId=itm.prodItemId 
	         				INNER JOIN send th ON th.sdNo=td.sdNo AND th.statusCode='P' AND th.fromCode=:fromCode 
	         					AND th.sendDate > '$lpcDate' AND th.sendDate <= '$dateFromYmd'
	         				WHERE itm.prodCodeId=:prodCodeId 
	          				GROUP BY 1, 2 ";
					$sql.="ORDER BY 1  ";
					//$sql.="LIMIT $start, $rows ";
					$stmt3 = $pdo->prepare($sql);	
					$stmt3->bindParam(':fromCode', $row['sloc']);	
					$stmt3->bindParam(':prodCodeId', $row['prodId']);	
					$stmt3->execute();
					while ($row3 = $stmt3->fetch()) {
						if($rowPerPage==40){
					...
					//Return Doc No
				$sql = "SELECT th.rtNo as docNo, th.returnDate as issueDate, SUM(itm.qty) as sumQty FROM product_item itm 
	          				INNER JOIN rt_detail td ON td.prodItemId=itm.prodItemId 
	         				INNER JOIN rt th ON th.rtNo=td.rtNo AND th.statusCode='P' AND th.fromCode=:fromCode 
	         					AND th.returnDate > '$lpcDate' AND th.returnDate <= '$dateFromYmd'
	         				WHERE itm.prodCodeId=:prodCodeId 
	          				GROUP BY 1, 2 ";
					$sql.="ORDER BY 1  ";
					//$sql.="LIMIT $start, $rows ";
					...
					//Delivery Doc No
				$sql = "SELECT th.doNo as docNo, th.deliveryDate as issueDate, SUM(itm.qty) as sumQty FROM product_item itm 
	          				INNER JOIN delivery_detail td ON td.prodItemId=itm.prodItemId 
	         				INNER JOIN delivery_header th ON th.doNo=td.doNo AND th.statusCode='P' 
	         					AND th.deliveryDate > '$lpcDate' AND th.deliveryDate <= '$dateFromYmd'
	         				INNER JOIN sale_header shd ON shd.soNo=th.soNo 
	         				INNER JOIN customer cust ON cust.id=shd.custId AND CASE WHEN cust.locationCode = 'L' THEN '8' ELSE 'E' END =:fromCode 
	         				WHERE itm.prodCodeId=:prodCodeId 
	          				GROUP BY 1, 2 ";
					$sql.="ORDER BY 1  ";
					//$sql.="LIMIT $start, $rows ";
