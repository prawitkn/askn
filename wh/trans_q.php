        ...
        <div class="box-body">			
			...
			<?php	
				if(isset($_GET['sendDate'])){
						...
						//TRUNCATE temp 
						echo "Get data...";
						$sql = "SELECT  `fromCode`, `issueDate`, `prodId`, `length`, `qty` FROM z_send_dtl";			
						$stmt = $pdo->prepare($sql);
						if($stmt->execute()){
							...
							while ($row = $stmt->fetch() )  {	
								...
								$irow=1;
								for ($i = 1; $i <= $qty; $i++) {
									$barcode=$fromCode.'-'.$prodId.'-'.(string)date("dmy", strtotime($issueDate)).'-'.substr('0000'.$irow,-5);
								    //Insert Header mysubstrsql from mssql
									$sql = "INSERT INTO z_product_item_q 
									(`prodItemId`,`prodCodeId`,`barcode`,`issueDate`,`NW`,`GW`,`qty`
									,`grade`,`gradeDate`,`gradeTypeId`)
									VALUES (:prodItemId, :prodCodeId, :barcode, :issueDate, 9.99, 9.99, :qty
									,0,:gradeDate,1)
									";														
									$stmt2 = $pdo->prepare($sql);
									$stmt2->bindParam(':prodItemId', $id);	
									$stmt2->bindParam(':prodCodeId', $prodId);
									$stmt2->bindParam(':barcode', $barcode);	
									$stmt2->bindParam(':issueDate', $issueDate);	
									$stmt2->bindParam(':qty', $length);	
									$stmt2->bindParam(':gradeDate', $issueDate);			
									$stmt2->execute();
						...
						(`prodItemId`,`sdNo`)
						SELECT prodItemId, '' FROM z_product_item_q
						";														
						$stmt = $pdo->prepare($sql);		
						$stmt->execute();


						$sql = "SELECT DISTINCT LEFT(barcode,1) AS `fromCode`, `issueDate` FROM z_product_item_q";			
						$stmt = $pdo->prepare($sql);
						if($stmt->execute()){
							while ($row = $stmt->fetch() )  {	
								$fromCode=$row['fromCode'];
								$issueDate=$row['issueDate'];
								$sdNo='SDI'.$fromCode.(string)date("dmy", strtotime($issueDate));

								$sql = "INSERT INTO z_send_q  
								(sdNo, sendDate, fromCode, toCode, statusCode)
								VALUES (:sdNo, :issueDate, :fromCode, 8, 'P') 
								";														
								$stmt2 = $pdo->prepare($sql);
								...
								$sql = "UPDATE z_send_detail_q a
								INNER JOIN z_product_item_q b on b.prodItemId=a.prodItemId AND LEFT(b.barcode,1)=:fromCode 
								SET a.sdNo=:sdNo
								";														
								$stmt2 = $pdo->prepare($sql);
								$stmt2->bindParam(':sdNo', $sdNo);	
