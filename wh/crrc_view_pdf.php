...
if( isset($_GET['rcNo']) ){
			...
			$sql = "SELECT dtl.id, dtl.prodItemId 
						, dtl.prodCode, dtl.barcode, dtl.grade, dtl.NW, dtl.GW, dtl.qty, dtl.issueDate, dtl.shelfCode
						, rtDtl.returnReasonCode, rtDtl.returnReasonRemark 
						FROM receive_detail dtl 
						LEFT JOIN rt_detail rtDtl on rtDtl.prodItemId=dtl.prodItemId AND rtDtl.rtNo=:refNo 
						WHERE rcNo=:rcNo  
						";			
						$stmt = $pdo->prepare($sql);	
						$stmt->bindParam(':rcNo', $hdr['rcNo']);
						$stmt->bindParam(':refNo', $hdr['sdNo']);
						$stmt->execute();	
