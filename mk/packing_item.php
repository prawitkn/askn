  ...
  <div class="content-wrapper">
    ...
    <div class="box box-primary">
        <div class="box-header with-border">
			<h3 class="box-title">View Sales Order No : <b><?= $hdr['orderNo']; ?></b></h3>
        ...
        <div class="box-body">
			...
			<div class="row">				
					...
					<div class="col-md-3">
						...
						Order No : <b><?= $hdr['orderNo']; ?></b><br/>
						Order Date : <b><?= $hdr['orderDate']; ?></b><br/>
		...
		<div class="row"><!-- row add items -->
				...
				<div class="box-body">
					<form id="form1" action="order_item_insert.php" method="post" class="form" novalidate>
					<input type="hidden" id="orderNo" value="<?= $hdr['orderNo']; ?>" />
						...
						<select id="prodCode" name="prodCode" class="form-control" >
							...
							<?php
							$sql = "SELECT `code`, `prodGroup`, `prodName`, `prodNameNew`, `prodDesc`, `prodPrice`, `appID` FROM `product` WHERE 1";							
							$stmt = $pdo->prepare($sql);		
							$stmt->execute();
			...
			<div class="row"><!-- row show items -->
				...
				<div class="box-tools pull-right">
				  ...
				  <?php
						$sql = "SELECT COUNT(*) AS COUNTSO FROM order_detail`
				...
				<div class="box-body">
				   <?php
						$sql = "
								...
								a.`discPercent`, a.`discAmount`, a.`netTotal`, a.`orderNo`,
								b.prodName
								FROM `order_detail` a
  ...
  //           
             $('#btn_submit').click (function(e) {
                ...
                if (params.netTotal==0 || params.netTotal==0.00 ){
                  ...
                  $.post("order_item_insert.php", params )
                              .done(function(data) {
									   if (data.status){  
										   $.smkAlert({
											text: data.message,
		   ...
		   //                                 position:'top-center'
            ...
            $('a[name=btn_row_delete]').click(function(){
				...
				$.smkConfirm({text:'คุณแน่ใจที่จะลบรายการนี้ใช่หรือไม่ ?',accept:'ลบรายการ', cancel:'ไม่ลบรายการ'}, function (e){if(e){
					$.post({
						url: 'order_item_delete_ajax.php',
						data: params,
						dataType: 'json'
					}).done(function (data) {					
						if (data.success){ 
			...
			$("#prodCode").on("change",function(e) {
			 ...
			 $('#btn_calc').click (function(e) {
				 ...
				 $discBaht = 0;
				 if($discPercent>0){
					 $discBaht = $total * $discPercent / 100;
				 }else{
					 $discPercent=0;
					 $discBaht = parseFloat($discAmount);
				 }				 
				 $netTotal = $total - $discBaht;
				 
				 $('#total').val($total.toFixed(2).toString().replace(/\B(?=(?=\d*\.)(\d{3})+(?!\d))/g, ',') );
				 $('#discAmount').val($discBaht.toFixed(2).toString().replace(/\B(?=(?=\d*\.)(\d{3})+(?!\d))/g, ',') );
