                   ...
                   <div class="info-box-content"> 
                       <?php
                        $sql ="SELECT SUM(b.total) AS total 
									FROM sale_header a
									inner join sale_detail b on a.soNo=b.soNo
									WHERE statusCode='P'
									AND year(saleDate)=:year 		
									";
						$stmt = $pdo->prepare($sql);
                ...
                <table class="table no-margin">
                  ...
                  <tbody>
				   <?php while ($row = mysqli_fetch_assoc($result)) { 
					...
					?>
                  <tr>
                    <td><a href="sale_view.php?id=<?=$row['id'];?>&soNo=<?=$row['soNo'];?>" target="_blank"><?= $row['id']; ?></a></td>
					<td><?= $row['custName']; ?></td>
					<td><?= $row['smName']; ?></td>
					<td>
						<?=$statusName;?>
					</td>
					<td><a href="sale_view.php?id=<?=$row['id'];?>&soNo=<?=$row['soNo'];?>" target="_blank"><?= $row['createTime']; ?></a></td>
                </tr>
                <?php  } ?>
                  </tbody>
                </table>
            ...
            <div class="box-body">
				<?php	
					$sql = "SELECT 
							...
							inner join sale_header oh on od.soNo=oh.soNo and oh.statusCode='P'							
								AND year(oh.saleDate)=:year ".
...
<?php
        $sql = "SELECT a.name, IFNULL(sum(b.forecast),0) as forecast
					, (SELECT IFNULL(sum(oh.total),0) FROM sale_header oh WHERE statusCode='P'
